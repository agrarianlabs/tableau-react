...
(function() {
    ...
    if (ss.isValue(info.customViews)) {
      ...
      if (list.length > 0) {
        for (var i = 0; i < list.length; i++) {
          ...
          if (ss.isValue(info.currentView) && info.currentView.id === customViewImpl.$presModel.id) {
  ...
  ss.initClass($tab__WorkbookImpl, $asm, {
    ...
    $removeCustomViewAsync: function WorkbookImpl$RemoveCustomViewAsync(cv) {
